...
(function (factory) {
    ...
    CityPicker.prototype = {
        ...
        render: function () {
            var p = this.getPosition(),
                ...
                dropdown = '<div class="city-picker-dropdown" style="left:0px;top:100%;' +
                    ...
                    '</div><div class="city-select-content">' +
        ...
        getText: function () {
            ...
            this.$dropdown.find('.city-select')
                .each(function () {
                    ...
                    if (item) {
                        text += ($(this).hasClass('province') ? '' : '/') + '<span class="select-item" ' +
                            'data-count="' + type + '" data-code="' + item.code + '">' + item.address + '</span>';
                    }
                });
            return text;
        },
        ...
        getVal: function () {
            ...
            this.$dropdown.find('.city-select').each(function () {
                ...
                if (item) {
                    // use code or address
                    var t = options.type === "address" ? item.address : item.code;
                    text += ($(this).hasClass('province') ? '' : '/') + t;
                }
            });
            return text;
        },
                ...
                $.each(districts, function (code, address) {
                    ...
                    if (type === PROVINCE) {
                        ...
                        for (var i = 0; i < address.length; i++) {
                            // use code or address
                            var t = options.type === "address" ? address[i].address : address[i].code;
                            if (t === value) {
                                matched = {
                                    code: address[i].code,
                        ...
                        // use code or address
                        var t = options.type === "address" ? address : code;
                        if (t === value) {
                            matched = {
                                code: code,
                                address: address
    ...
    CityPicker.DEFAULTS = {
        simple: false, // address format
        responsive: false,
        placeholder: '请选择地址',
        level: 'district', // min level
        province: '',
        city: '',
        district: '',
        street: '',
