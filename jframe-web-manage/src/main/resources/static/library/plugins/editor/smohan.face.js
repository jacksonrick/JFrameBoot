$(function () {
    ...
    $.fn.smohanfacebox = function (options) {
        ...
        //创建表情框
        var faceimg = '';
        for (var i = 0; i < total; i++) {  //通过循环创建表情，可扩展
            faceimg += '<li><a href="javascript:void(0)"><img src="static/library/plugins/editor/images/face/' + (i + 1) + '.gif" face="<emt>' + (i + 1) + '</emt>"/></a></li>';
        }
        ;
        ...
        $facepic.unbind().click(function () {
            ...
            $("#" + options.textid).unbind().insertContent($(this).attr("face"));
            $btn.removeClass('in');
        });
        //关闭表情层
    ...
    $.fn.extend({
        replaceface: function (faces) {
            for (var i = 0; i < total; i++) {
                faces = faces.replace('<emt>' + (i + 1) + '</emt>', '<img src="static/library/plugins/editor/images/face/' + (i + 1) + '.gif">');
            }
            $(this).html(faces);
        }
    });
